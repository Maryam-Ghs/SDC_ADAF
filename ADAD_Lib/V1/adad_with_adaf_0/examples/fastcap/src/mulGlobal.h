...
/* SRW --
 * The ualloc package causes an immediate bus error on Solaris 8
 * systems.  I doubt that there is any real advantage in using an
 * internal allocator, particularly one that is likely to bugger
 * the standard allocators and crash the program.  If you really
 * want ualloc, define USE_UALLOC in the makefile.
 */
#ifdef USE_UALLOC
